...
function getNameLocationInGlobalDirectiveComment(sourceCode, comment, name) {
    var namePattern = new RegExp("[\\s,]".concat((0, escapeRegExp_1.escapeRegExp)(name), "(?:$|[\\s,:])"), 'gu');
    // To ignore the first text "global".
    namePattern.lastIndex = comment.value.indexOf('global') + 6;
    // Search a given variable name.
    var match = namePattern.exec(comment.value);
    // Convert the index to loc.
    var start = sourceCode.getLocFromIndex(comment.range[0] + '/*'.length + (match ? match.index + 1 : 0));
    var end = {
        line: start.line,
